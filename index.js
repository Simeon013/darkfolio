...
const skill_BOXES = document.querySelectorAll('.skill-card__box');
const ACTIVE_LINK_CLASS = 'active';
const BREAKPOINT = 576;

let currentskillBG = null;
...
window.addEventListener('scroll', ()=>{
  const sections = document.querySelectorAll('#heroHeader, #skills, #projects, #contact');
...
// Handles the hover animation on skills section
skill_BOXES.forEach(skill => {
  const moveBG = (x, y) => {
    Object.assign(currentskillBG.style, {
      left: x + 'px',
      top: y + 'px',
    })
  }
  skill.addEventListener('mouseenter', (e) => {
    if (currentskillBG === null) {
      currentskillBG = skill.querySelector('.skill-card__bg');
    }
    moveBG(e.clientX, e.clientY);
  });
  skill.addEventListener('mousemove', (e) => {
    const LEFT = e.clientX - skill.getBoundingClientRect().left;
    const TOP = e.clientY - skill.getBoundingClientRect().top;
    moveBG(LEFT, TOP);
  });
  skill.addEventListener('mouseleave', () => {
    const IMG_POS = skill.querySelector('.skill-card__illustration')
    const LEFT = IMG_POS.offsetLeft + currentskillBG.getBoundingClientRect().width;
    const TOP = IMG_POS.offsetTop + currentskillBG.getBoundingClientRect().height;
